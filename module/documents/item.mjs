...
import { normalizeRollModes } from "../lib/compatibility.mjs";
...
export class NechronicaItem extends Item {
  ...
  getChatData() {
    ...
    if (itemData.type === "bodypart") {
      if (NECHRONICA.attackTypes[itemData.attack]) {
        result.props.push(
          game.i18n.localize(NECHRONICA.attackTypes[itemData.attack]),
        );
      }
      if (
        itemData.type === "bodypart" &&
        NECHRONICA.locations[itemData.location]
      ) {
        result.props.push(
          game.i18n.localize(NECHRONICA.locations[itemData.location]),
        );
      }
      if (NECHRONICA.timings[itemData.timing]) {
        result.props.push(
          game.i18n.localize(NECHRONICA.timings[itemData.timing]),
        );
      }
      if (itemData.range) {
        result.props.push(
          game.i18n.localize("NECH.Range") + ` ${itemData.range}`,
        );
      }
      if (itemData.cost !== undefined) {
        result.props.push(
          game.i18n.localize("NECH.Cost") +
            ` ${itemData.cost} ` +
            game.i18n.localize("NECH.AP"),
        );
      }
      ...
      result.props.push(
        game.i18n.localize("NECH.Target") + ` ${itemData.target}`,
      );
      if (itemData.type) {
        result.props.push(
          game.i18n.localize("NECH.Type") + ` ${itemData.type}`,
        );
      }
      result.props.push(
        game.i18n.localize("NECH.Madness") + ` ${itemData.madness}`,
      );
    }
    return result;
  }
  ...
  async roll({
    ...
    const templateData = {
      ...
      rollModes: normalizeRollModes(CONFIG.Dice.rollModes),
      config: NECHRONICA,
      ...
      if (combatant && isAttack) {
        ...
        if (ap != null && ap < 1) {
          ui.notifications?.warn(
            game.i18n.format("Warnings.MissingAP", {
        ...
        if (itemData.used && !this.isRepeatable && !ev.ctrlKey) {
          ui.notifications?.warn(
            game.i18n.format("Warnings.AlreadyUsed", {
              actor: this.actor?.name,
              part: this.name,
            }),
          );
    ...
    console.trace("DEBUG");
        form = await new Promise((resolve) => {
          new Dialog({
            title:
              game.i18n.localize(`${isAttack ? "NECH.Attack" : "NECH.Check"}`) +
              `: ${this.name}`,
    ...
    console.trace("DEBUG");
      ...
      if ((isAttack && hasAttack) || isCheck) {
        ...
        if (
          isAttack &&
          getProperty(rollData, `modifiers.${itemData.attack}.attack`)
        ) {
          parts.push(`@modifiers.${itemData.attack}.attack`);
          templateData.system.extraProps.push(
            ...(actor?.sourceDetails?.[
              `system.modifiers.${itemData.attack}.attack`
            ]?.map((m) => m.name) ?? []),
          );
        }
        // Roll bonus from form
        rollData.rollBonus =
          Number(form?.find('[name="roll-bonus"]').val()) + rollBonus;
        if (rollData.rollBonus) parts.push("@rollBonus");

        // Damage bonus from form
        rollData.damageBonus =
          Number(form?.find('[name="damage-bonus"]').val()) + damageBonus;
        ...
        roll = await Roll.create(parts.join("+"), rollData).evaluate({
          async: true,
        });
    ...
    if (rollType == "MESSAGE") {
      ...
      const spine =
        this.name == game.i18n.localize("NECH.DefaultItems.name.Spine");
      if (actor && typeof actor.spendAp === "function") {
